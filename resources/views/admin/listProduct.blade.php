                        ...
                        <tbody>
                            @foreach ($products  as $data )
                             <tr>
                                <th>{{ $data->id }}</th>
                                <td>{{ $data->name}}</td>
                                <td>{{ $data->category->name ?? 'الصنف غير موجود' }}</td>
                                <td><img src="{{ $data->getMedia("avtar")->first()?->getUrl()}}" width="50"  height="50"></td>
                                ...
                                <td>{{ $data->qualty }}</td>
                                <td>{{ $data->price }}</td>
                                <td><a href="editProduct/{{ $data->id }} "><img src="assets/images/admin/edit.png" width="30"></a></td>
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">إغلاق</button>
                    ...
                    @method('DELETE')
                    <a  href="deleteRecordProduct/{{ $data->id }}" class="btn btn-danger">حذف</a>
